...
$selectedModulePaths = if ($selectedModules)
{
    $selectedModules.FullName
}
else
{
    @()
...
$outputFileName = "backup_for_ai_$timestamp.txt"
...
foreach ($item in $allItems)
{
    ...
    if ($isInsideSelectedModule -or $isRootFile)
    {
        ...
        if (Is-TextFileToInclude -FileItem $item)
        {
            ...
            if ($isRootFile)
            {
                $includedRootFiles.Add($item.Name)
            }
        }
...
$includedModuleNames = if ($selectedModules)
{
    $selectedModules.Name -join ', '
}
else
{
    "None"
